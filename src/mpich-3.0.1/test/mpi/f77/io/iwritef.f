...
C Try writing the file, then check it
     ...
     & MPI_MODE_CREATE, MPI_INFO_NULL, fh, ierr )
                ...
                do k=1, b ,2
                   ...
                   call mpi_file_iwrite( fh, buf, n, MPI_INTEGER, req(1   &
     &), ierr )
                   if (ierr .ne. MPI_SUCCESS) then
                       errs = errs + 1
                       if (errs .le. 10) then
     ...
     & ierr )	
                       ...
                       enddo
                       nreq = nreq + 1
                       call mpi_file_iwrite( fh, buf2, n, MPI_INTEGER,    &
     & req(2), ierr )
                       if (ierr .ne. MPI_SUCCESS) then
                           errs = errs + 1
                           if (errs .le. 10) then
...
C Now, open the same file for reading
     ...
     & MPI_MODE_CREATE, MPI_INFO_NULL, fh, ierr )
                ...
                do k=1, b ,2
                   ...
                   call mpi_file_iread( fh, buf, n, MPI_INTEGER, req(1)   &
     &, ierr )
                   if (ierr .ne. MPI_SUCCESS) then
                       errs = errs + 1
                       if (errs .le. 10) then
     ...
     & ierr )	
                       ...
                       enddo
                       nreq = nreq + 1
                       call mpi_file_iread( fh, buf2, n, MPI_INTEGER,     &
     & req(2), ierr )
                       if (ierr .ne. MPI_SUCCESS) then
                           errs = errs + 1
                           if (errs .le. 10) then
