...
C Try writing the file, then check it
     ...
     & MPI_MODE_CREATE, MPI_INFO_NULL, fh, ierr )
                ...
                if (r .eq. s-1) then
                    call mpi_ssend( MPI_BOTTOM, 0, MPI_INTEGER, dest, 1   &
     &, comm, ierr )
                endif
                do k=1, b
                    call mpi_recv( MPI_BOTTOM, 0, MPI_INTEGER, src, k,    &
     & comm, MPI_STATUS_IGNORE, ierr )
                    do i=1, n
                        buf(i) = r*n + (k-1)*n*s + i-1
                    enddo
                    call mpi_file_iwrite_shared( fh, buf, n,              &
     & MPI_INTEGER, req, ierr )
                    if (ierr .ne. MPI_SUCCESS) then
                        errs = errs + 1
                        if (errs .le. 10) then
                            call MTestPrintError( ierr )
                    ...
                    if (r .eq. s-1) then
                        call mpi_ssend( MPI_BOTTOM, 0, MPI_INTEGER,       &
     & dest, k+1, comm, ierr )
                    else
                        call mpi_ssend( MPI_BOTTOM, 0, MPI_INTEGER,       &
     & dest, k, comm, ierr )
                ...
                enddo
                if (r .eq. 0) then
                    call mpi_recv( MPI_BOTTOM, 0, MPI_INTEGER, src, b+1   &
     &, comm, MPI_STATUS_IGNORE, ierr )
                endif
...
C Now, open the same file for reading
     ...
     & comm, MPI_STATUS_IGNORE, ierr )
                    do i=1, n
                        buf(i) = - (r*n + (k-1)*n*s + i)
                    enddo
                    call mpi_file_iread_shared( fh, buf, n, MPI_INTEGER   &
     &, req, ierr )
                    if (ierr .ne. MPI_SUCCESS) then
                        errs = errs + 1
                        if (errs .le. 10) then
